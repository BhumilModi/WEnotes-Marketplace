...
const style = {
    ...
    headerText: `px-5 text-[20px] text-[#0085FF] cursor-pointer border-b-[4px] border-[#0085FF] pb-2`,
    headerText2: `px-5 text-[20px] text-[#9A9A9A] hover:text-[#0085FF] cursor-pointer`,
    list: `flex flex-col`,
    item: `pt-3`
}
                    ...
                    <div className={style.list}>
                        {
                            notes.map((note, index) => (
                                <div key={index} className={style.item}>
                                    <BoughtNotes img={note.image} header={note.header} description={note.description} author={note.author} price={note.price} holder1={note.holder1} holder2={note.holder2} h1price={note.h1price} h2price={note.h2price}></BoughtNotes>
                                </div>
                            ))
                        }
                    </div>
